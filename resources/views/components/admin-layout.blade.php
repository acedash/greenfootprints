...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-slate-50 text-slate-900 selection:bg-emerald-500 selection:text-white" style="font-family: 'Inter', sans-serif;">
        ...
        <!-- Mobile top bar -->
        <div class="lg:hidden h-16 bg-white border-b border-slate-200 flex items-center justify-between px-5 sticky top-0 z-30 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-emerald-500 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/30">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                </div>
                <h1 class="font-bold text-lg tracking-tight text-slate-800">Admin<span class="text-emerald-500">Panel</span></h1>
            </div>
            <button id="mobile-menu-btn" class="p-2 rounded-xl text-slate-500 hover:bg-slate-100 transition-colors">
        ...
        <!-- Mobile overlay -->
        <div id="mobile-overlay" class="lg:hidden fixed inset-0 bg-black/40 z-20 hidden backdrop-blur-sm"></div>

        <div class="flex h-screen lg:h-auto overflow-hidden lg:overflow-visible">

            <!-- Sidebar -->
            <aside id="sidebar" class="fixed lg:sticky top-0 left-0 h-full w-72 bg-white border-r border-slate-200 flex flex-col z-30 shadow-[4px_0_24px_rgba(0,0,0,0.04)] -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out lg:h-screen">
                <!-- Logo -->
            ...
            <!-- Main Content -->
            <main class="flex-1 min-h-screen overflow-y-auto relative bg-[#f8fafc]">
                <!-- Desktop Header -->
                <header class="h-16 lg:h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-5 lg:px-10 hidden lg:flex justify-between items-center sticky top-0 z-10">
                ...
                <div class="p-5 lg:p-10 max-w-7xl mx-auto">
                    {{ $slot }}
                </div>
            </main>
